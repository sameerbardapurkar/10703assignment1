    ...
    for n in range(0, max_iterations):
      ...
      for i in range(0, env.nS):
        ...
        for j in range(0, len(transition)):
          ...
          isterminal = transition[j][3]
          new_value += probability*reward
          if(isterminal == False):
            new_value += probability*gamma*value_func[nextstate]
      ...
      for i in range(0, env.nS):
        ...
        for j in actions:
          ...
          for k in range(0, len(transition)):
            ...
            nextstate = transition[k][1]
            isterminal = transition[k][3]
            #print(reward, nextstate, isterminal)
            new_value += probability*reward
            if(isterminal == False):
              new_value += probability*gamma*value_func[nextstate]
...
print("   carried out %d policy improvement steps and %d policy evaluations" 
      % (pol_count, val_count))
print("b.")
print_policy(policy.reshape(24,27), actions_dict)
print(value_func.reshape(24,27))
plt.imshow(value_func.reshape(24,27))
plt.colorbar()
#plt.savefig('/home/sameer/10-703/assignment1/results/q_2_partc_c_4_pol_iter.eps',
         #bbox_inches='tight')
plt.clf()
...
print("d. Finished value iteration in %s s" % (time.time() - start_time))
print("   %d iterations were required" % num_iter)
plt.imshow(val_func_alternate.reshape(24,27))
plt.colorbar()
plt.savefig('/home/sameer/10-703/assignment1/results/q_3_val_iter.eps',
         bbox_inches='tight')
plt.clf()
policy_alternate = value_function_to_policy(env, gamma, val_func_alternate)
print("h.")
print_policy(policy_alternate.reshape(24,27), actions_dict)
